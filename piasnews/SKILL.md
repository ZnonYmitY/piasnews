...
2. If Piasnews static data is available, read `data/items.json`, `data/daily.json`, or `data/rss.xml` next.
...
When `data/items.json` and `data/daily.json` are available, prefer them over live searching for normal news briefs and daily counts. These files are generated by `scripts/fetch_piasnews.py` and refreshed by GitHub Actions.
