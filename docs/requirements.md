...
- GitHub Actions first runs `scripts/translate_zh_argos.py` after data collection as a build-time offline Chinese fallback, then runs `scripts/apply_immersive_translations.py`. Captured Immersive Translate mappings overwrite `title_zh` and `summary_zh`, then deterministic auto-repair fixes accumulated low-risk terminology issues. Argos does not call an online translation API and does not consume model tokens; Immersive mappings remain higher priority when present. Approved manual cases are no longer default production overrides; they only mark samples for future training/evaluation.
