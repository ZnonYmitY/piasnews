...
- `data/social.json` may be generated by the X API, user-exported JSON, an Agent-Reach-style local collection tool, or another external workflow. External workflows must store only public-post short paraphrases, timestamps, links, and account attribution, not long verbatim content.
- The local Agent-Reach collection entrypoint is `scripts/collect_agent_reach_social.py`. It reads `piasnews/references/x-sources.json`, calls the local Agent-Reach-selected `twitter-cli` backend, writes `/tmp/piasnews-agent-reach-social.json`, and imports it into `data/social.json`. If authentication is unavailable, it reports source failure and does not invent content.
- Every fan-source card must show account attribution and a removal-on-rights-request notice.
