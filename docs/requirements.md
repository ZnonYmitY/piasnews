...
- Oscar Piastri's Instagram account is a `daily_core` official source. Agent-Reach does not currently expose an Instagram backend; a logged-in local Chrome session can read profile-grid links and individual post/reel detail pages, including captions, `time[datetime]`, and public engagement metadata. The IG collector should use the flow `profile links -> detail page published_at/text -> latest-three-days filter -> data/social.json import`; profile-grid links alone are not enough because the homepage does not expose stable timestamps. When IG is provided by another trusted backend, the Supabase social collector may merge that compact JSON through `PIASNEWS_INSTAGRAM_INPUT_URL`, `PIASNEWS_INSTAGRAM_INPUT_JSON`, or a JSON POST body so the local scheduler does not need to open Instagram.
...
- The local Agent-Reach collection entrypoint is `scripts/collect_agent_reach_social.py`. It reads `piasnews/references/x-sources.json`, tries `OpenCLI → X Web GraphQL → twitter-cli`, normalizes OpenCLI `media_urls` / `media_posters` into image, video, and poster fields, and imports the result into `data/social.json`. When the browser bridge is unavailable it reuses local cookies from `~/.agent-reach/config.yaml`, preventing a successful text-only legacy response from silently removing media. If authentication is unavailable, it reports source failure and does not invent content.
...
- A fresh social snapshot for the same URL must not replace an existing non-empty `image_url`, `video_url`, or `video_poster_url` with an empty value. Compact imports carry only the latest three-day discovery window for GitHub to merge into the seven-day retained dataset, and record collector version, generation time, item count, media-item count, and latest-item time; snapshot equality ignores generation time so timestamps alone do not trigger deployment.
- `agent-reach/compact-social` must use the current v2 schema. Missing or older schema versions are incompatible and ignored so an outdated runtime cannot continue injecting text-only degraded snapshots; the existing seven-day dataset remains available.
- The GitHub data workflow compares `data/social.json` before and after collection. If a retained same-URL item loses any known media field, the run must stop before publishing. Items that naturally leave the seven-day retention window are not regressions.
- The macOS runtime copy fast-forwards its code immediately before a real collection and fails closed when stale, dirty, or conflicted. Scheduled normalization writes to a temporary output instead of continually dirtying the runtime repository's `data/social.json`.
- X collection can migrate to an always-on mini host, VPS, Supabase Edge Function, or external scheduler by generating compact social JSON, updating `PIASNEWS_SOCIAL_INPUT_JSON`, exposing `PIASNEWS_SOCIAL_INPUT_URL`, or pushing external compact input into the Supabase social collector, then triggering `Update Piasnews Data` through GitHub API. Prefer a home/always-on local environment over data-center IPs because X may apply stricter risk controls to VPS traffic.
...
- GitHub Actions first runs `scripts/translate_zh_argos.py` after data collection as a build-time offline Chinese fallback. If `PIASNEWS_LLM_TRANSLATION_API_KEY` is configured, it then runs `scripts/translate_zh_llm_mapping.py` to generate missing `data/immersive_translations.zh.json` entries with engine `piasnews_llm_translation`; the low-cost recommended variables are `PIASNEWS_LLM_TRANSLATION_BASE_URL=https://api.deepseek.com` and `PIASNEWS_LLM_TRANSLATION_MODEL=deepseek-v4-flash`. `scripts/apply_immersive_translations.py` applies both LLM and Immersive mappings, then deterministic auto-repair fixes accumulated low-risk terminology issues. Argos does not call an online translation API and does not consume model tokens; Immersive Translate remains the fallback mapping path when LLM mapping is unavailable or fails. Approved manual cases are no longer default production overrides; they only mark samples for future training/evaluation.
...
Hot words follow the objective MVP pattern “person + concrete action or quote + necessary context.” The deterministic layer retains seven days of context, calculates heat from only the latest 72 hours with time decay, caps repeated contributions per source, and clusters with configurable keyword rules plus conservative title similarity. Before clustering it removes platform wrappers, URLs, repost prefixes, and transport-only account handles; the most informative member names the merged event. Handle-only, link-only, emoji-only, and generic captions such as `video here` are held as “title needed” even when media exists. At most two organic fan-only events from one account may appear publicly; cross-source events, official posts, session results, and active editorial overrides are exempt, while capped candidates remain in the workbench. Public and review events must have globally unique `event_id` values or the build fails closed. The model does not directly select or rank events in this version. Ambiguous items stay separate unless an authorized operator corrects them.

The workbench stores editorial changes in a separate overlay and supports `viewer`, `editor`, `publisher`, and `admin` roles. Editors can save drafts; publishers and admins can activate an override. Configurable event fields include the bilingual hot word, heat, editorial position, source labels, public visibility, and change reason. An editorial position only reorders an event that still meets `minimum_heat`; it cannot displace the session-result hard rule or keep a zero-heat event on the ranking. The newest publication wins a same-position editorial collision. Publicly hidden events remain visible and labeled in the workbench. Each event has an editable content list; every item owns its source metadata, original URL, image, video, and poster. Writes are serialized, and optimistic `updated_at` checks reject stale same-event edits while allowing different-event changes to merge in sequence. In the topic detail view, the entire content card—including source, title, summary, and media preview—opens the original source, with no separate “View original” action. Vague media-dependent posts without captured media are held as “media needed”; semantically empty posts are held as “title needed,” and fan-account cap candidates are labeled separately. Comments are deferred.

Discovery remains three days and merges retained items until the window gradually reaches seven days, without an initial backfill. Search matches the original English body without translating it. Incremental translation cache entries are retained for eight days. A dedicated cron triggers the daily ranking refresh at 07:00 Asia/Shanghai, with additional refreshes after each completed race-weekend session plus a 15-minute confirmation buffer. The 15-minute gate reads only the checked-in calendar and latest handled session reference; it requests OpenF1 only when a newly completed session is due, not during the ordinary 07:00 refresh. The workflow reads Oscar's structured position or DNF/DNS/DSQ status from OpenF1 `session_result` and creates the hard-ranked first event independently of news discovery. Anonymous reads are a non-live-window fallback. If a live-access request returns 401 and the GitHub Actions secrets `PIASNEWS_OPENF1_USERNAME` and `PIASNEWS_OPENF1_PASSWORD` are configured, the backend exchanges them at OpenF1's fixed `/token` endpoint for a one-hour OAuth2 token; credentials and tokens never enter logs, public data, or Pages. The latest valid result remains stored across daily, translation apply-only, and editorial rebuilds for traceability and retries. Once a newer session is pending, however, the older session is no longer presented as the latest hard-ranked event. A same-session transient retry may retain its current display, and refetching it preserves the first-ranked timestamp and 24-hour lifetime. A missing new result remains unhandled so the gate retries.
...
- Local Immersive Translate capture is available as an optional no-LLM enhancement: after syncing GitHub data, the local runner builds the online workbench, opens Chrome, sends the Immersive Translate shortcut `Option+A`, scrolls the pages, captures translated DOM mappings, and can commit/push the mapping. Mapping publication triggers `update-piasnews.yml` with `apply_only=true`, so the workflow applies translations, auto-repair, badcase audit, and Pages deployment without refetching new data.
