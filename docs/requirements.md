...
Discovery remains three days and merges retained items until the window gradually reaches seven days, without an initial backfill. Search matches the original English body without translating it. Incremental translation cache entries are retained for eight days. A dedicated cron triggers the daily ranking refresh at 07:00 Asia/Shanghai. A 15-minute Cloudflare heartbeat reads the published calendar, daily timestamp, and latest handled session reference, then dispatches GitHub Actions only when the 07:00 build is overdue or a newly completed race-weekend session has passed its 15-minute confirmation buffer. Non-race days therefore do not continuously start Actions or call a result provider. The workflow prefers OpenF1 `session_result`; if it is blocked by a live-window 401, still pending, or temporarily unavailable, the collector validates the finished Formula 1 Live Timing archive. A `Finished` session with a valid Oscar mapping and complete ranking is published immediately without an extra provisional label, while a later provider record may revise the same session reference. Optional OAuth credentials and tokens remain backend-only. The latest validated result is always hard-ranked first, while every earlier validated session result remains visible for its own configured minimum 24-hour lifetime. A pending or attempted newer session never creates an empty result gap, a successful newer result changes the top order without deleting still-live earlier results, and refetching preserves each record's first-ranked timestamp.
