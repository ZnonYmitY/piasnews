...
为了避免每次下载 Excel，仓库也支持把候选同步到飞书多维表格 Base。飞书 Base 作为人工审核界面，仓库 CSV 作为可追溯源数据。
...
8. 下一次 GitHub Actions 会把飞书 Base 的 approved 行写回 `translation_review.csv`，翻译脚本读取 approved 样本，优先覆盖同一 `source_text`。
9. 当样本积累到 100-300 条，再导出为微调数据集。
