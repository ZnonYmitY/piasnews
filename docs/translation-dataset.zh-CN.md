...
4. GitHub Actions 调用 `scripts/apply_immersive_translations.py`，把 approved 案例和 `data/immersive_translations.zh.json` 中的沉浸式翻译映射覆盖到 `title_zh` / `summary_zh`。
5. GitHub Actions 调用 `scripts/audit_translations.py` 完整遍历最终展示中文，执行对抗式审查。
6. 新发现的坏例进入 `data/translation_candidates.csv`，本轮新增候选同时导出为 `translation_candidates_latest.csv` 和 `translation_candidates_latest.xlsx`。
7. 如果配置了飞书 Base，GitHub Actions 调用 `scripts/sync_feishu_translation_base.py`，把本轮新增候选同步到飞书审核表。
8. 你在飞书审核表中检查 `英文原文`、`当前中文`、`建议中文`。如果认可，把 `审核状态` 改为 `approved`；如果暂不处理，保持 `pending`；如果不需要进入确认集，可改为 `ignored`。
9. 下一次 GitHub Actions 会把飞书审核表的 approved 行写回 `translation_review.csv`，映射应用脚本读取 approved 样本，优先覆盖同一 `source_text`。
10. 同一次 workflow 会把 `translation_review.csv` 中的 approved 样本同步到飞书“翻译案例库”表，作为人工可读的案例库镜像。
11. 当样本积累到 100-300 条，再导出为微调数据集。
