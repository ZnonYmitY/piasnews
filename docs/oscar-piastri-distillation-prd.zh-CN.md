...
**文档状态：** 蒸馏 v0.3 进行中；人物知识库与谣言核验层已建立，但 X 全历史覆盖、双人审核和冻结回归尚未完成，未开放公开运行
...
**当前实现进度（v0.3）：** 独立的 `piastri-fan-companion/` 蒸馏包现包含 33 条正式证据、10 条候选判断规则、7 张风格卡、9 类边界兜底、18 条人物知识事实、11 条谣言核验条目、26 个冻结前评测题、Correction Log、来源覆盖清单和结构校验脚本。正式证据中有 14 份比赛采访/发布会、4 份 team radio、7 份 Oscar 官方 X 时间留出样本，以及 8 份长访谈或官网回顾。X 年度断点已完成到 2023 年；2017–2023 年共取得 1,333 条，连同最新时间线的 20 条无重叠发言，实际观察总量为 1,353 条。2024-01-01 至 2026-04-07 仍因会话限流未完成，所有人物判断规则仍为 `candidate`；完成双人审核和冻结回归前，只用于内部候选测试，不对外宣称完成 L3 判断蒸馏。
...
X 全历史的完成口径是：从账号创建日到当前日期逐年检索，每个时间窗无错误且低于单窗上限；达到上限的时间窗继续拆月；最终按 post ID 去重，并分别统计原创、回复、转发和不可见/删除缺口。当前实际看到 1,353 条公开发言：2017–2023 年年度断点共 1,333 条，以及 2026-04-08 至 2026-09-02 的最新时间线 20 条；2024-01-01 至 2026-04-07 先后因 X HTTP 429 和 OpenCLI `BROWSER_CONNECT` 暂未完成。因此产品和文档标记为 `partial_rate_limited_and_opencli_error`，不得称为全量。
...
谣言台账保存标准化主张，而不是耸动标题。首版覆盖 Alpine 合同争议、McLaren 合同期限、离队传闻、未公开合同条款、队友关系、车队偏袒、Monza 团队指令、papaya rules、私生活和真假引语或无线电。每个条目包含结论类型、置信度、时效、已知事实、未知项、来源、复查触发器和中英安全答复。
...
- 有时效与来源的公开人物事实库，以及按主张、证据状态和复查触发器维护的谣言台账（首版已完成 18 条事实、11 条谣言）。
