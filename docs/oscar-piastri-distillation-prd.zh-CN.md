...
**文档状态：** 蒸馏 v0.4 进行中；X 公开可见索引的年度日期窗口已完成，人物知识库与谣言核验层已扩充，但无线电人工听审、双人审核和冻结回归尚未完成，未开放公开运行
...
**当前实现进度（v0.4）：** 独立的 `piastri-fan-companion/` 蒸馏包现包含 46 条正式证据、10 条候选判断规则、7 张风格卡、9 类边界兜底、21 条人物知识事实、15 条谣言核验条目、31 个冻结前评测题、Correction Log、来源覆盖清单、X 风格分析和结构校验脚本。正式证据中有 14 份比赛采访/发布会、4 份 team radio、13 份 2022–2025 Oscar 官方 X 风格训练候选、7 份 2026 X 时间留出样本，以及 8 份长访谈或官网回顾。X 年度断点已覆盖账号创建日至 2026-09-05 的所有日期窗口，按 ID 去重共 1,661 条：1,245 条主帖、416 条回复和 719 条带媒体内容，未观察到转发。这里的“完成”只指公开可见搜索索引，不能恢复采集前已删除、受限或未被索引的内容。所有人物判断规则仍为 `candidate`；完成无线电人工听审、双人审核和冻结回归前，只用于内部候选测试，不对外宣称完成 L3 判断蒸馏。
...
flowchart LR
  ...
  B -->|人物事实| L[读取人物知识并检查时效]
  B -->|网传 / 真伪核验| M[标准化主张并匹配谣言台账]
  B -->|完全无关| D[选择边界兜底]
  B -->|高风险或伤害| E[直接拒绝]
  C --> F[形成内部判断并选择最佳回复]
  L --> F
  M --> N[第三人称事实结论、未知项与截至日期]
  D --> G[英文角色输出]
  E --> G
  F --> G
  G --> H{用户是否输入中文}
...
| 公开人物事实 | 生日、81 号由来、职业时间线、首个领奖台 | 从人物知识库回答；易变字段先检查时效 |
| 谣言与真伪核验 | Alpine 合同、离队、偏袒、队友闹翻、真假 radio | 标准化主张后进入谣言台账；用第三人称事实口径，不进行角色表演 |
...
| --- | --- | --- | --- |
| Oscar 官方 X | 短句、压缩程度、普通词汇、轻微幽默、图文配合 | 风格训练候选与时间留出 | 多数帖子依赖图片/视频；官方账号不等于逐条确认本人撰写，不能单独推出比赛判断或私人心理 |
...
X 全历史的完成口径是：从账号创建日到当前日期逐年检索，每个时间窗无错误且低于单窗上限；达到上限的时间窗继续拆月；最终按 post ID 去重，并分别统计原创、回复、转发和不可见/删除缺口。当前从公开可见搜索索引实际取得 1,661 条，覆盖 2016-05-09 至 2026-09-05 的所有年度日期窗口，包括 1,245 条主帖、416 条回复和 719 条带媒体内容。产品和文档标记为 `complete_for_visible_search_index`，同时显式保留采集前已删除、受限或未被索引内容无法恢复的限制。2023–2025 的公开账号输出明显变短且更依赖媒体，但该趋势可能受平台、内容规划或社交团队影响，只能用于按页面形态调整表达长度，不能宣称为私人性格演化或逐条本人执笔。

### 4.2 人物知识库与谣言台账

人物知识库不是新闻全文仓库。它保存能稳定复用的最小事实单元，包括生日、出生地、81 号由来、青训三连冠、F1 时间线、关键里程碑、当前车队和公开合同状态。每条事实包含英文与中文答案、权威来源、稳定性、截至日期、复查日期和使用限制。

事实分为三类：`stable` 用于历史与身份；`seasonal` 用于当前车队、合同和公开职业关系；`live` 用于赛果、积分、赛程、处罚和突发新闻。`live` 内容不写死在人物库中，而是每次回答前调用 Piasnews 当前数据。公开职业关系只能支持“工作关系仍在继续”，不能推断私下友谊、矛盾或内心状态。

谣言台账保存标准化主张，而不是耸动标题。当前覆盖 Alpine 合同争议、McLaren 合同期限、离队传闻、未公开合同条款、队友关系、车队偏袒、Monza 团队指令、papaya rules、私生活、真假引语或无线电、官方账号逐条作者、所谓“没有情绪”、永不质疑车队指令，以及“匈牙利首胜不算”的事实/评价混淆。每个条目包含结论类型、置信度、时效、已知事实、未知项、来源、复查触发器和中英安全答复。

辟谣不是简单输出“真/假”。系统区分“当前措辞与权威记录直接矛盾”“包含真实元素但误导”“截至某日尚无可靠支持”“来源无法核验”“公开材料仍有争议”“旧信息已被更新”和“私人边界”。动态转会与合同问题必须写明截至日期；未找到公告不能升级成永久假消息。涉及私生活时不继续搜索、列人名或整理粉丝推测。
...
| PersonKnowledge | 事实键、值、中英回答、稳定性、截至/复查日期、来源、限制 | 回答公开人物事实并控制时效 |
| RumorItem | 标准化主张、别名、结论、已知/未知、来源、时效、复查触发器 | 辟谣而不放大传闻 |
...
所有反馈先进入 Correction Log，并被分为事实错误、知识库时效、谣言结论、人物判断错配、风格不自然、兜底不合适、翻译偏差、评测争议或安全问题。分类之后只修改最小对象：事实错误修对应人物事实；新官方信息只更新相关动态谣言；“太啰嗦”修改对应意图的风格卡；某个学术问题不该回答则修改“无关知识”路由或兜底；这些变化都不能顺带改写其他场景的人格倾向。
...
- 30 条以上带元数据的第一方表达与比赛事实材料（数量已达 46，X 可见索引日期覆盖已闭合；仍需无线电人工听审）。
- 有时效与来源的公开人物事实库，以及按主张、证据状态和复查触发器维护的谣言台账（当前 21 条事实、15 条谣言）。
