...
- 数据 workflow 基于下一场比赛元数据生成 `data/next-race.ics` 和 `data/next-weekend.ics`；公开页面提供一键导入 iCalendar 兼容日历应用的入口，包括 Apple Calendar、Google Calendar、Outlook 等。不同产品的导入交互不同，但底层 `.ics` 文件格式一致。
...
- GitHub Actions 在数据采集后运行 `scripts/apply_immersive_translations.py`，先把沉浸式翻译采集到的映射覆盖到 `title_zh` 和 `summary_zh`，再执行 deterministic auto-repair 修正已沉淀的确定性术语坏例；缺少映射的新内容保留抓取脚本生成的确定性中文或英文兜底。Approved 人工案例不再作为默认生产覆盖源，只作为后续训练 / 评估样本状态；Argos 仅保留为手动 fallback / 模型评估工具，不再作为默认生产翻译器。
