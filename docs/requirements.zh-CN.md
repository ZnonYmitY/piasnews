...
- `data/items.json`、`data/daily.json` 和 `data/rss.xml` 已存在。
...
## 去年今日
仅在有维护好的历史来源、用户提供的历史上下文，或当前官方信息明确提到同日重要事件时展示。没有大事时不展示。

## 数据
只保留一行：新增 N，官方 N，媒体 N，传闻 N，X N。

## 备注
- 仅在数据过期、来源失败或 X 不可用等情况需要解释时展示。
```
...
- 展开数据面板：只在用户要求统计、深读版，或数据对理解新闻有帮助时展开来源和分类明细。
