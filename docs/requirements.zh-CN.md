...
- 日常热榜由独立 cron 在北京时间 07:00 触发；比赛周在练习赛、排位、冲刺赛和正赛结束并留出 15 分钟确认时间后刷新。Cloudflare 每 15 分钟只预检公开赛历、日更时间和最近成绩；只有 07:00 日更漏跑或新赛段达到确认时间且仍未处理时才启动 GitHub Actions，因此非比赛日不持续运行 Actions，也不访问成绩接口。OpenF1 `session_result` 是确认源；匿名请求遇到直播窗口 401、结果仍待发布或临时不可用时，采集器校验已经结束的 F1 Live Timing 结构化归档，并先发布明确标注“暂定”的名次/状态。下一次北京时间 07:00 日更会重试 OpenF1 并覆盖为确认成绩。可选 OAuth 凭据和 token 只用于后端。结果通过硬规则置顶第一；支持正常名次和 DNF、DNS、DSQ，同一成绩重复采集不重置 24 小时计时。
