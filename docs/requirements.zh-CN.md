...
- X 等增强源只作为可选能力，在用户提供自己的凭证或项目配置自有访问能力后再接入。
...
- 维护 X / Instagram 账号源表，并在配置访问能力后生成社交动态流。
- 支持后续统计每日新增信息数量。
...
- 粉丝日报支持速读版、日报版和粉丝源三个 Tab；日报版合并原标准版和深读版的高价值结构，粉丝源展示已抓取到的 X / Instagram 发帖与转帖。
...
- `public/` 已实现公开粉丝日报，包含速读、日报和粉丝源三个 Tab、右上角中英切换，并分别显示新闻数据更新时间和 X / IG 粉丝源更新时间。
...
- 定时任务每 6 小时抓取公开来源，并在 10 分钟后配置一条备用 cron。这里是低成本的 GitHub Actions 兜底，不是有状态的漏触发检测；重复运行是安全的，因为数据未变化时不会产生提交。
...
- 页面提供速读、日报和粉丝源三个 Tab；速读和日报与 Skill 中的日报模式保持一致，粉丝源读取已生成的 X / Instagram 社交动态流。
- 页面提供右上角中文 / English 切换。中文模式优先读取 `title_zh` 和 `summary_zh`；如果存在中文标题，新闻链接文字也应使用中文标题，原始英文标题只作为溯源信息展示。
- 速读和日报读取同一份 `data/items.json`、`data/daily.json` 和已审核 `data/history.json`，不复制新闻数据；粉丝源读取 `data/social.json`，展示 `source_type=x|instagram` 的社交动态条目。
- 页面读取 `data/calendar.json` 展示下一场大奖赛、比赛周时间和下一赛段倒计时；练习赛、冲刺排位、冲刺赛、排位赛或正赛进行中时切为从开始计算的正计时，结束后自动切到下一赛段。赛历属于赛程元数据，不受新闻三天窗口限制，也不能用于填充日报。
- 数据 workflow 基于赛历元数据生成 `data/next-race.ics`、`data/next-weekend.ics` 和 `data/full-season.ics`；公开页面提供一键导入 iCalendar 兼容日历应用的入口，包括 Apple Calendar、Google Calendar、Outlook 等。不同产品的导入交互不同，但底层 `.ics` 文件格式一致。移动端按钮必须可换行，避免四个入口在窄屏互相挤压。
- 页面必须展示 `data/items.json` 的 `generated_at` 对应北京时间、`data/social.json` 的 `generated_at` 对应北京时间、`data/social.json.items` 中最新有效内容的 `published_at` 对应北京时间，以及当前三天窗口。
- 速读版最多 5 条；无传闻时不显示传闻提醒，也不显示数据面板。
- 日报版合并原标准版与深读版：保留今日重点、话题合并、官方、媒体、可选传闻、可选往日回顾和底部轻量统计；`daily_core` 社交来源进入普通日报信息流，不单独展示 X / 社交观察栏目。
- “今日重点”只展示直接命中 Oscar Piastri / Piastri / OP81 的条目；泛 F1、泛车队或赛程内容不能仅凭来源可信度进入今日重点。重点排序应综合发布时间、赛段进度、比赛结果/排位/练习赛相关性、官方/可信来源和传闻降权，避免当前已进入后续赛段时仍优先展示旧练习赛信息。
- 社交条目的链接文字必须包含可读内容摘要，不能只显示 `X 发帖：@账号`；中文界面优先展示中文概括，无法可靠概括时可退回英文原文片段。
...
- 本地 Agent-Reach 采集入口由 `scripts/collect_agent_reach_social.py` 提供。它读取 `piasnews/references/x-sources.json`，默认按 `OpenCLI → X Web GraphQL → twitter-cli` 的顺序读取公开时间线，并将 OpenCLI 的 `media_urls` / `media_posters` 归一化为图片、视频和封面字段，再导入 `data/social.json`。浏览器桥接不可用时复用 `~/.agent-reach/config.yaml` 中的本机 cookies，避免旧后端成功返回正文却静默丢失媒体；未认证时只输出失败状态，不伪造内容。
...
- 倒计时每秒在浏览器本地更新，优先指向下一条赛段 session；session 进行中显示正计时，session 结束后切换到下一条。赛历接口失败时继续发布仓库中上一份有效数据。
...
- 该文件作为用户自有 X / Instagram 访问或未来 V1/V2 采集器的发现指南，只保留在仓库配置中；Pages artifact 发布的是 `data/social.json`，不是账号表。
- 单独统计 X 来源的每日新增数量。
...
- 摘要应使用用户选择的语言。
- 速读版不展示数据面板；日报版底部保留轻量统计，避免让数据打断正文阅读。

## 9. GitHub 同步

### 9.1 热榜 MVP（暂不发布）

当前版本把原“速读”Tab 调整为合并热榜。官方、媒体和粉丝内容共用一个事件，依靠稳定的 `event_id` 关联；榜单不显示“新 / 热 / 爆”等状态，只显示来源标签 `官`、`媒`、`粉` 和整数热度。点击事件进入独立热点详情，热点词固定在顶部，下方把官方/媒体内容与粉丝讨论分区展示。

热点词采用客观模板“人物＋具体动作或原话＋必要场景”。例如：`Oscar 在花絮视频中比划乌龟和鲨鱼`。MVP 不持久化 `entities`、`scene` 等中间字段，也不要求所有低置信结果人工审核：自动规则无法可靠合并时宁可拆成独立事件，有权限的运营人员可在工作台纠正。

热榜自动层遵循以下规则：

- 候选信息保留最近 7 天，但热度只计算最近 72 小时，并按 `0–6h / 6–24h / 24–48h / 48–72h` 分段衰减。
- 官方、媒体、粉丝分别计算分项热度；单一来源最多取 3 条高热信息，防止刷屏；总热度取最强分项为主，叠加较弱分项。
- MVP 使用可配置关键词规则和标题相似度聚类，不调用大模型直接筛选或排序。后续只有在 case 证明必要时，才考虑让模型辅助生成客观热点词或判断边界样本。
- 聚类前移除平台包装、链接、转发前缀和仅用于传输的账号标记；热点词从同一事件中语义最完整的成员提取。只有账号、链接、表情或 `video here` 等泛化指代的内容，即使带媒体也进入“待补标题”，不得直接以账号名上榜。
- 同一粉丝账号的纯自然热点最多公开 2 条；跨来源事件、官方内容、赛段成绩和已启用人工覆盖不受此限制。受限候选保留在工作台，便于运营判断是否人工发布。
- 每次生成必须保证公开榜与审核队列中的 `event_id` 全局唯一；无法消解的重复 ID 直接使构建失败，不发布可能串线的榜单。
- 日常热榜由独立 cron 在北京时间 07:00 触发；比赛周在练习赛、排位、冲刺赛和正赛结束并留出 15 分钟确认时间后刷新。每 15 分钟运行的 gate 只读取仓库内的赛历和最近成绩引用，只有新赛段达到确认时间且仍未处理时才请求 OpenF1；普通 07:00 日更不请求成绩接口。场次成绩由独立的 OpenF1 `session_result` 结构化接口获取，不再依赖新闻标题命中；支持正常名次和 DNF、DNS、DSQ。匿名请求只作为非直播窗口的兜底；GitHub Actions 可通过 Secrets `PIASNEWS_OPENF1_USERNAME`、`PIASNEWS_OPENF1_PASSWORD` 在直播访问返回 401 时向固定的 OpenF1 `/token` 端点换取一小时临时 OAuth2 token。凭据和 token 仅用于后端请求，不进入日志、静态数据或 Pages。结果可用时通过硬规则生成“人物＋场次＋名次/状态”热点词，并以高于人工置顶的优先级排在第一。最近一次有效成绩跨普通日更、翻译 apply-only 和后台配置发布保留在成绩文件中，供追溯与重试；检测到更新场次仍待处理时，旧场次不再作为最新结果硬置顶。同一场次的临时失败仍可保留已有展示，同一成绩重复采集保留首次入榜时间，不重置 24 小时计时。接口暂未返回或请求失败时不把新赛段标记为已处理，并在后续 gate 继续重试。
- 图片优先读取文章或社交帖的 OG 图片；视频仅保存封面和外链，不做代理、下载或转码。
- 媒体归属于单条关联内容，不直接挂在事件上。每条内容可分别保存原链接、图片、视频和视频封面；详情页以卡片形式露出。
- 热点详情中的每条内容卡整体可点击（包括来源、标题、摘要和媒体预览），直接跳转原始来源，不再设置独立的“查看原内容”按钮。
- 对 `I'm never leaving this app` 等依赖图片/视频才能理解的短文案，缺少媒体时不生成具体事件名、不进入前台，转入工作台“待补媒体”；只有账号、链接、表情或泛化指代的内容则无论是否带媒体都转为“待补标题”。补齐可核验媒体和明确标题后才可发布。

人工工作台采用独立覆盖层，不改写采集原文。权限分为：

- `viewer`：查看热榜、历史审核和统计。
- `editor`：保存热榜草稿。
- `publisher`：让热榜覆盖立即生效，并执行历史审核。
- `admin`：拥有全部权限。

可配置项包括热点词中英文、热度覆盖、人工指定名次、来源标签、前台隐藏和调整理由。人工指定名次只改变仍达到普通热度门槛的事件顺序，不绕过 `minimum_heat`：当前低于 8（包括热度 0）即自动下榜；赛后成绩硬规则优先于任何人工名次。相同人工名次以最近发布的修改优先。从前台隐藏只影响公开热榜，事件仍保留在后台并显示隐藏标记。事件下提供当前内容列表，可新增、删除或编辑单条内容，并为该内容配置来源类型、标题、摘要、原链接、图片、视频和视频封面。每次修改记录操作人、时间和理由。多人操作时写入队列串行执行，同一事件通过 `updated_at` 做乐观版本校验，旧版本提交返回冲突并要求刷新；不同事件可依次合并。评论讨论功能不在本次 MVP 范围内。

配套数据规则：新内容仍只发现最近 3 天，通过每天合并逐步把现有数据窗口扩展到 7 天，不做首次回填；正文搜索直接匹配英文原文，不翻译正文；译文只处理增量，缓存保留 8 天。
...
- GitHub Pages 根路径发布速读、日报、粉丝源三 Tab 粉丝日报，分别显示新闻数据和 X / IG 粉丝源更新时间，并在每次采集工作流后同步更新。
- 速读和日报共用静态新闻数据，粉丝源读取 `data/social.json` 动态流；页面渲染不调用大模型；速读不展示数据面板，日报底部保留轻量统计。
- 网页展示下一条 F1 赛段倒计时或进行中正计时、北京时间赛程、官方赛历链接，以及下一场正赛 / 比赛周末的 iCalendar 导入链接；赛历随采集工作流更新。
