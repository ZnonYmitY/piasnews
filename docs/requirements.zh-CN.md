...
- 日常热榜由独立 cron 在北京时间 07:00 触发；比赛周在练习赛、排位、冲刺赛和正赛结束并留出 15 分钟确认时间后刷新。Cloudflare 每 15 分钟只预检公开赛历、日更时间和最近成绩；只有 07:00 日更漏跑或新赛段达到确认时间且仍未处理时才启动 GitHub Actions，因此非比赛日不持续运行 Actions，也不访问成绩接口。成绩优先读取 OpenF1 `session_result`；匿名请求遇到直播窗口 401、结果仍待发布或临时不可用时，采集器校验已经结束的 F1 Live Timing 结构化归档。只要赛段状态为 `Finished`，Oscar 车手映射和完整排名均通过校验，就直接同步名次/状态，不额外显示“暂定”；后续提供方记录仍可按同一场次引用覆盖更新。可选 OAuth 凭据和 token 只用于后端。支持正常名次和 DNF、DNS、DSQ；最新有效成绩通过硬规则置顶第一，较早的每条有效赛段成绩分别保留至少 24 小时。新赛段结果尚未通过校验时继续展示上一条成绩，不产生空窗；拿到新结果后只更新榜首顺序，不提前删除仍在保留期内的旧赛段成绩，重复采集也不重置各自计时。
