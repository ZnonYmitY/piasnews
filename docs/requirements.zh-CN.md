...
- 粉丝日报支持速读版和日报版两种输出长度；日报版合并原标准版和深读版的高价值结构。
...
- `public/` 已实现公开粉丝日报，包含速读和日报两个 Tab、右上角中英切换，并显示数据更新时间。
...
- 页面提供速读、日报两个 Tab，并与 Skill 中的日报模式保持一致。
...
- 两种网页日报共用静态数据且不调用大模型；速读和日报版默认不展示数据面板。
