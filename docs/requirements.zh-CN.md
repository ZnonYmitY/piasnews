...
- GitHub Actions 在数据采集后运行 `scripts/apply_immersive_translations.py`，先把沉浸式翻译采集到的映射覆盖到 `title_zh` 和 `summary_zh`，再执行 deterministic auto-repair 修正已沉淀的确定性术语坏例；缺少映射的新内容保留抓取脚本生成的确定性中文或英文兜底。Approved 人工案例不再作为默认生产覆盖源，只作为后续训练 / 评估样本状态；Argos 仅保留为手动 fallback / 模型评估工具，不再作为默认生产翻译器。翻译审查只导出仍需人工关注且已有 `suggested_zh` 的候选；生产 workflow 不再把候选同步成飞书 Base pending 队列，也不再把缺失候选标记为 ignored。
