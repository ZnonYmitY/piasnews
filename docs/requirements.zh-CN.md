...
- 生成并保存每日新增数量统计。
- 历史事件候选、人工审核信号和语义字段存放在 `data/history.json`；检索策略存放在 `piasnews/references/history-retrieval.json`，两者随 Pages artifact 一起发布。
...
历史事件结构采用事实、人工审核信号和语义字段三层。新增候选默认保持 `pending`，不会进入日报：
