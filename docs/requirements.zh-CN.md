...
- 数据 workflow 基于下一场比赛元数据生成 `data/next-race.ics` 和 `data/next-weekend.ics`；公开页面提供一键导入 Apple Calendar 及其他 iCalendar 兼容日历应用的入口。
...
- GitHub Actions 在数据采集后运行 `scripts/apply_immersive_translations.py`，先导入 approved 审核案例，再把沉浸式翻译采集到的映射覆盖到 `title_zh` 和 `summary_zh`；缺少映射的新内容保留抓取脚本生成的确定性中文或英文兜底。Argos 仅保留为手动 fallback / 模型评估工具，不再作为默认生产翻译器。
...
- 网页展示下一场 F1 正赛倒计时、北京时间赛程、官方赛历链接，以及下一场正赛 / 比赛周末的 iCalendar 导入链接；赛历随采集工作流更新。
