...
- 非比赛周每天刷新一次；比赛周在练习赛、排位、冲刺赛和正赛结束并留出 15 分钟确认时间后刷新。场次成绩由独立的 OpenF1 `session_result` 结构化接口获取，不再依赖新闻标题命中；支持正常名次和 DNF、DNS、DSQ。结果可用时通过硬规则生成“人物＋场次＋名次/状态”热点词，并以高于人工置顶的优先级排在第一。最近一次有效成绩跨普通日更、翻译 apply-only 和后台配置发布保留，直到被更新的有效赛段成绩替换，或实际在榜满 24 小时自动退出；同一成绩重复采集保留首次入榜时间，不重置计时。接口暂未返回或请求失败时不把新赛段标记为已处理，并在后续 gate 继续重试；等待期间仍展示未满 24 小时的上一有效成绩。
