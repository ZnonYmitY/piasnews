...
- `data/items.json`、`data/daily.json`、`data/rss.xml` 和 `data/history.json` 已存在。
...
- `piasnews/references/history.md` 已存在，用于维护可选历史事件上下文。
- 尚未添加托管后端。
...
- 生成并保存每日新增数量统计。
- 维护好的历史事件存放在 `data/history.json`，并随 Pages artifact 一起发布。
...
## 去年今日
仅在 `data/history.json`、用户提供的历史上下文，或当前官方信息明确提到同日重要事件时展示。没有大事时不展示。
```
...
- 去年今日：仅在 `data/history.json` 或其他维护/用户提供的历史来源支持时展示。
