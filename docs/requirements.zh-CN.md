...
- `public/` 已实现公开粉丝日报，包含速读、标准和深读三个 Tab，并显示数据更新时间。
...
GitHub Pages 根路径提供面向所有粉丝的只读日报页，不增加后端数据库或在线模型服务。

- 页面提供速读、标准、深读三个 Tab，并与 Skill 中的三种日报模式保持一致。
- 三个版本读取同一份 `data/items.json`、`data/daily.json` 和已审核 `data/history.json`，不复制新闻数据。
...
- 标准版按官方、媒体和传闻分区；无内容的可选分区直接省略。
- 深读版按类别合并话题，展示来源可信度、明日关注和数据面板。
...
- GitHub Pages 根路径发布三 Tab 粉丝日报，显示数据更新时间，并在每次采集工作流后同步更新。
- 三种网页日报共用静态数据且不调用大模型；速读和标准版不展示数据面板。
