...
- 粉丝源 Tab 不展示后台账号表，只展示抓取或导入到 `data/social.json` 的发帖与转帖短摘要、时间、链接和账号归属；未配置访问能力时，不应伪造社交动态。
- `data/social.json` 可由 X API、用户导出的 JSON、Agent-Reach 类本地采集工具或其他外部流程生成。外部流程必须只写入公开动态的短摘要、时间、链接和账号归属，不保存大段原文。
- 本地 Agent-Reach 采集入口由 `scripts/collect_agent_reach_social.py` 提供。它读取 `piasnews/references/x-sources.json`，调用本机 Agent-Reach 选中的 `twitter-cli` 后端，生成 `/tmp/piasnews-agent-reach-social.json`，再导入 `data/social.json`。未认证时只输出失败状态，不伪造内容。
- 粉丝源每条卡片必须展示 `引用自 @账号` 和 `如有侵权请联系删除。`
...
- 速读和日报共用静态新闻数据，粉丝源读取 `data/social.json` 动态流；页面渲染不调用大模型，速读和日报版默认不展示数据面板。
