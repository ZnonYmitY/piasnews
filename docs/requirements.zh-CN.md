...
- 非比赛周每天刷新一次；比赛周在练习赛、排位、冲刺赛和正赛结束并留出确认时间后刷新。场次刷新能从来源中可靠识别 Oscar 名次时，通过硬规则生成“人物＋场次＋名次”热点词并置顶；没有可靠名次证据时不猜测。
...
可配置项包括热点词中英文、热度覆盖、人工指定名次、来源标签、前台隐藏和调整理由。填写人工名次 1 可强插热榜第一；相同名次以最近发布的修改优先。从前台隐藏只影响公开热榜，事件仍保留在后台并显示隐藏标记。事件下提供当前内容列表，可新增、删除或编辑单条内容，并为该内容配置来源类型、标题、摘要、原链接、图片、视频和视频封面。每次修改记录操作人、时间和理由。多人操作时写入队列串行执行，同一事件通过 `updated_at` 做乐观版本校验，旧版本提交返回冲突并要求刷新；不同事件可依次合并。评论讨论功能不在本次 MVP 范围内。
