...
- 非比赛周每天刷新一次；比赛周在练习赛、排位、冲刺赛和正赛结束并留出 15 分钟确认时间后刷新。场次成绩由独立的 OpenF1 `session_result` 结构化接口获取，不再依赖新闻标题命中；支持正常名次和 DNF、DNS、DSQ。结果可用时通过硬规则生成“人物＋场次＋名次/状态”热点词，并以高于人工置顶的优先级排在第一，直到下一次成功重建热榜。接口暂未返回或请求失败时不把该赛段标记为已处理，后续 gate 继续重试。
