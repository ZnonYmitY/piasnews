...
After news and social data are fetched, GitHub Actions runs `scripts/apply_immersive_translations.py`. It applies mappings from `data/immersive_translations.zh.json` to overwrite `title_zh` and `summary_zh`, then runs deterministic auto-repair for accumulated low-risk terminology fixes. New content without a mapping keeps the deterministic Chinese summary or English fallback generated by the fetch scripts until local Immersive Translate capture fills the mapping.
