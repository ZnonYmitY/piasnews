...
- **速读版**：最多 5 条，适合快速看今天有没有大事；默认不展开数据面板。
- **标准版**：默认粉丝日报，一句总览、今日重点、官方动态、媒体报道、传闻雷达和一行数据。
- **深读版**：按话题合并同类报道，补充来源可信度、明日关注和更完整的数据面板。

数据面板默认保持轻量：只有一行统计。只有当用户明确要求统计、深读版，或数据本身解释新闻价值时，才展开来源/分类明细。

“去年今日”是可选模块。只有存在明确重要事件或维护好的历史来源时才展示；没有大事时不硬凑。
...
- **Short**: Up to 5 bullets for a quick check; no expanded data panel by default.
- **Standard**: Default fan daily with one-line summary, key points, official updates, media coverage, rumor radar, and compact stats.
- **Deep**: Groups similar coverage into topic cards and adds source-confidence notes, next watch points, and a fuller data panel.

Keep the data panel lightweight by default: one compact stats line only. Expand source/category details only when the user asks for stats, asks for deep mode, or the data itself explains the story.

"On this day last year" is optional. Show it only when there is a clear meaningful event or a maintained historical source; omit it when there is no notable item.
