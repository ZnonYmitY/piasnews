...
- 粉丝日报支持速读版、日报版和粉丝源三个 Tab；日报版合并原标准版与深读版的高价值结构，粉丝源用于展示 X / IG 参考账号和未来社交动态。
- 默认中文输出，英文提问时输出英文。
- 明确标注非官方来源、传闻和未验证信息。
...
- **粉丝源**：第三个网页 Tab，用于展示人工维护的 X / Instagram 参考账号；后续接入抓取后展示发帖与转帖的短摘要、时间、链接和账号归属。
...
- 页面新增粉丝源 Tab，读取 `data/x-sources.json` 展示维护好的 X / IG 账号表；未来社交动态接入后也在这里展示。
...
当前维护的 X / Instagram 参考源位于 `piasnews/references/x-sources.json`：
...
- X / IG 参考源：https://znonymity.github.io/piasnews/data/x-sources.json
...
- **粉丝源 Tab**：账号库和页面展示已接入；真正抓取 X / IG 发帖与转帖仍需要用户或项目配置可用访问方式。
...
- Supports short, daily, and fan-source tabs. The daily mode merges the useful parts of the previous standard and deep modes; the fan-source tab lists X / IG reference accounts and future social updates.
...
- **Fan Sources**: A third website tab for the maintained X / Instagram account list. Future social collection can show short post/repost summaries, timestamps, links, and account attribution here.
...
- The page provides short, daily, and fan-source tabs. Short and daily share the same verified static data; fan sources reads `data/x-sources.json`.
...
The maintained X / Instagram source list is `piasnews/references/x-sources.json`:
...
- X / IG source list: https://znonymity.github.io/piasnews/data/x-sources.json
...
- **Fan-source tab**: account list and page rendering are implemented; actual X / IG post and repost collection still requires configured access.
