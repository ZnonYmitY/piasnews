...
当前版本是 **V1 静态数据 + 公开粉丝日报 + 历史审核台**：粉丝使用 Skill 或直接打开网页都不需要提供私有 token。仓库通过 GitHub Actions 定时生成最近 3 天的新闻数据和重大事件候选；网页随每次采集自动更新，维护者可以在静态审核台确认候选后写入正式历史库。比赛直播窗口内的 OpenF1 成绩读取可由维护者在后端配置可选认证，凭据不会下发到网页。
...
自动层会先去掉社交平台包装、链接、转发前缀和用于聚类的账号标记，再判断事件语义；只有账号、链接、表情或 `video here` 等泛化指代的内容，即使带视频也不会自动拿账号名充当热点词，而会进入工作台“待补标题”。热点词优先从同一事件中信息最完整的成员提取。同一粉丝账号的纯自然热点最多公开 2 条，跨来源事件、官方内容、赛段成绩和人工启用覆盖不受此限制；被限流候选仍留在工作台。生成阶段还会校验 `event_id` 全局唯一，避免详情入口串线。

日常热榜由独立 cron 在北京时间 07:00 触发。Cloudflare Worker 每 15 分钟只预检已发布的赛历、日更时间和最近成绩；正常情况下，非比赛日不启动 GitHub Actions，也不访问成绩接口。只有 07:00 日更漏跑，或新赛段结束并经过 15 分钟确认缓冲且仍未处理时，Worker 才调度仓库内的正式 gate 和精简赛果更新。成绩优先读取 OpenF1 `session_result`；匿名请求遇到直播窗口 401、正式结果仍待发布或临时网络错误时，采集器会读取 F1 Live Timing 已结束赛段的结构化归档。只要 `SessionStatus=Finished` 且车手、名次和完整排名通过校验，就直接同步成绩，不额外显示“暂定”；后续 OpenF1 记录可按同一场次引用覆盖更新。若 F1 源站拒绝 GitHub Actions 出口，采集器只通过带专用密钥的 Worker 内部中转读取严格白名单内的 Index、SessionStatus、DriverList 和 TimingData 文件；中转不是开放代理。可选的 GitHub Actions Secrets `PIASNEWS_OPENF1_USERNAME` 和 `PIASNEWS_OPENF1_PASSWORD` 仍可用于 OpenF1 OAuth，凭据和一小时 token 只留在后端进程。结果可用时生成“人物＋场次＋名次/状态”热点；最新成绩硬置顶第一，较早的有效成绩各自继续留榜至少 24 小时。新赛段结果尚未拿到时仍保留上一条成绩，不制造空窗；拿到新结果后只更新榜首顺序，不提前删除仍在保留期内的旧赛段成绩，重复采集也不会重置计时。
...
The current release combines **V1 static data, a public fan daily, and a history review console**. Fans can use the Skill or open the website without supplying a private token. GitHub Actions refreshes recent news and major-event candidates, redeploys the public daily, and lets the maintainer approve candidates into the formal history library. Maintainers may optionally configure backend-only OpenF1 authentication for result reads during live-session windows; those credentials are never shipped to the webpage.
...
Before clustering, the deterministic layer removes social wrappers, URLs, repost prefixes, and account handles used only as transport metadata. Handle-only, link-only, emoji-only, and generic captions such as `video here` are held as “title needed” even when media exists, so an account name cannot become the public hot word. The most informative member names a merged event. At most two organic fan-only events from one account may appear publicly; cross-source events, official posts, session results, and active editorial overrides are exempt, while capped candidates remain available in the workbench. Generation also fails closed on duplicate `event_id` values.

After a race-weekend session and its 15-minute confirmation buffer, the workflow first reads Oscar's structured result from OpenF1 `session_result`. A Cloudflare heartbeat checks the published calendar, daily timestamp, and handled result; under normal public-state availability it dispatches GitHub Actions only for a missed 07:00 Beijing refresh or an unhandled completed session. If OpenF1 is blocked by a live-window 401, still pending, or temporarily unavailable, the collector validates the finished Formula 1 Live Timing archive. Once `SessionStatus=Finished` and the driver and complete ranking checks pass, that result is published directly without an additional provisional label; a later OpenF1 record may revise the same session reference. When the F1 origin blocks GitHub's egress, the collector uses a dedicated-secret Worker relay restricted to the four required static resource types; it is not a general proxy. Optional OAuth credentials remain backend-only. The newest normal position or DNF/DNS/DSQ status becomes the hard-ranked first event, while each earlier validated session result remains in the ranking for its own at-least-24-hour lifetime. A pending session never creates a result gap, and a newer result changes the top order without prematurely deleting still-live earlier results or resetting their timers.
