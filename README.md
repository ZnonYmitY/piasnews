...
当前版本已升级到 **V1 静态数据版**：不依赖托管后端、不要求 X API、不使用我们的私有 token 或第三方付费额度。仓库会通过 GitHub Actions 定时生成最近 3 天的 `items.json`、`daily.json` 和 `rss.xml`，粉丝的 Agent 可以优先读取静态数据。
...
- 最新条目：[data/items.json](data/items.json)
- 每日统计：[data/daily.json](data/daily.json)
- RSS Feed：[data/rss.xml](data/rss.xml)
...
The current release is a **V1 static-data release**. It does not require a hosted backend, X API access, our private tokens, or paid third-party API quota. GitHub Actions refreshes `items.json`, `daily.json`, and `rss.xml` for the latest 3-day window, and agents can read static data first.
...
- Latest items: [data/items.json](data/items.json)
- Daily stats: [data/daily.json](data/daily.json)
- RSS feed: [data/rss.xml](data/rss.xml)
