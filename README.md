...
日常热榜由独立 cron 在北京时间 07:00 触发。Cloudflare Worker 每 15 分钟只预检已发布的赛历、日更时间和最近成绩；非比赛日不启动 GitHub Actions，也不访问成绩接口。只有 07:00 日更漏跑，或新赛段结束并经过 15 分钟确认缓冲且仍未处理时，Worker 才调度仓库内的正式 gate 和完整更新。成绩以 OpenF1 `session_result` 为确认源；匿名请求遇到直播窗口 401、正式结果仍待发布或临时网络错误时，采集器会读取 F1 Live Timing 已结束赛段的结构化归档，在严格校验车手、名次和完赛状态后先发布带“暂定”标记的结果。后续北京时间 07:00 日更会再次查询 OpenF1 并用确认成绩覆盖。可选的 GitHub Actions Secrets `PIASNEWS_OPENF1_USERNAME` 和 `PIASNEWS_OPENF1_PASSWORD` 仍可用于 OpenF1 OAuth，凭据和一小时 token 只留在后端进程。结果可用时生成“人物＋场次＋名次/状态”热点并硬置顶第一；支持正常名次以及 DNF、DNS、DSQ，重复采集不会重置 24 小时计时。
...
After a race-weekend session and its 15-minute confirmation buffer, the workflow reads Oscar's structured result from OpenF1 `session_result`. A Cloudflare heartbeat first checks the published calendar, daily timestamp, and handled result; it dispatches GitHub Actions only for a missed 07:00 Beijing refresh or an unhandled completed session, so non-race days do not continuously start Actions. If OpenF1 is blocked by a live-window 401, still pending, or temporarily unavailable, the collector validates the finished Formula 1 Live Timing archive and publishes a clearly labelled provisional result. The next 07:00 refresh retries OpenF1 and replaces that fallback with the confirmed result. Optional OpenF1 OAuth credentials remain backend-only. A normal position or DNF/DNS/DSQ status becomes the hard-ranked first event, and refetching the same session does not reset its 24-hour timer.
