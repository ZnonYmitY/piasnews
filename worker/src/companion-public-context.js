...
function validatedResultUrl(item) {
  ...
  if (item.source === "Formula 1 Live Timing") {
    if (!Number.isSafeInteger(item.session_key) || item.session_key <= 0
        || (item.provisional != null && typeof item.provisional !== "boolean")
...
function buildSession(data, sourceStatus, nowMs, publicSources) {
  ...
  const isF1Timing = latest.source === "Formula 1 Live Timing";
  const provider = isF1Timing ? "Formula 1 Live Timing" : "OpenF1";
  ...
  if (resultAvailable) {
    publicSource = sourceEntry({
      ...
      source: isF1Timing ? (provisional ? "Formula 1 Live Timing provisional result" : "Formula 1 Live Timing result") : "OpenF1 session_result", data_provider: provider,
      sourceType: "official", evidenceTier: isF1Timing ? "official_live_timing_archive" : "public_results_provider",
      ...
      facts: {
        ...
        temporal_scope: isF1Timing
          ? (provisional
            ...
            : "Latest completed result in this snapshot from the finished live-timing archive; later provider revisions may replace it.")
      ...
      answer_limits: ["Match the requested race and session type; a practice result is not the previous Grand Prix result.",
        provisional ? "Label this result as provisional; it is not the confirmed classification." : "A completed result snapshot is not a championship table or proof of live progress."],
    });
...
function normalizeRecordedResult(item, data, sourceStatus, nowMs) {
  ...
  const isF1Timing = item.source === "Formula 1 Live Timing";
  const provider = isF1Timing ? "Formula 1 Live Timing" : "OpenF1";
  ...
  return sourceEntry({
    ...
    facts: {
      ...
      temporal_scope: isF1Timing
        ? (provisional
          ...
          : "Recorded completed F1 Live Timing result for this named session as last observed; later provider revisions may replace it.")
    ...
    answer_limits: ["Match the requested event and session type; a practice result is not a Grand Prix race result.",
      ...
      provisional ? "This live-timing archive result must be labelled provisional." : "A recorded result is not a championship table. A fresh calendar may help resolve event order but cannot establish participation or completion."],
  });
}
