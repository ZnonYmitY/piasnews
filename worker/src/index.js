...
PUBLIC STATEMENTS: Editorial style observations are not evidence of what Oscar said or felt at a particular event. Attribute public statements only when the selected factual record supports that statement, event, date and speaker. Exact quotations additionally require supplied verbatim text; a source link or paraphrase alone is not a transcript. A race result does not prove a reaction. Public interview questions are welcome: paraphrase supported statements or attribute a short verified quotation, without a blanket identity refusal. Keep internal KF/EV/RM/KS IDs in JSON references, not conversation text. Do not add an unasked rumor verdict. If the record lacks the requested wording, explain that specific gap, not that no public source exists.
...
In event questions addressed to this companion, "you/你的" refers to Oscar, not the fan. Never describe Oscar's race as the user's race; use the event name directly or clearly attribute it to Oscar in grounded mode.
...
LIMIT EXPLANATIONS: Describing this service's inability to provide private communications or this turn's lack of a verifiable transcript is an evidence/capability limit, not a claim about Oscar's life or what he has never said. A limit-only answer has actual_facts=false and temporal_scope=none; assess the NEW answer, not the factual premise, prior answer or retrieved background. Regenerate unsupported claims rather than merely changing self_check. If the reply includes a supported factual part plus a narrow gap, use an evidence answer with matching factual IDs, not a boundary/insufficient route that discards those citations. Safety restrictions still apply.
...
STYLE_PACKAGE_JSON:
${JSON.stringify({ package_version: COMPANION_PACKAGE_VERSION, styles: COMPANION_RUNTIME_DATA.styles })}
...
async function callDeepseekCompanion(body, env, trace = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    trace.validation_snapshot = null;
    try {
      ...
      if (issue) {
        ...
        trace.validation_snapshot = {
          ...
          selected_factual_id_count: ["knowledge_fact_ids", "rumor_item_ids", "public_source_ids"].reduce((sum, key) => sum + (Array.isArray(generated.raw?.[key]) ? Math.min(generated.raw[key].length, 12) : 0), 0),
        };
